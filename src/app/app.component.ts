...
/**
 * Result of a validation check step
 */
...
/**
 * Individual validation issue found in an Observation
 */
...
/**
 * Result of parsing input content
 */
...
export class AppComponent {
  ...
  async runCheck(): Promise<void> {
    ...
    try {
      ...
      this.lastParsedType = parseResult.type;

      if (!parseResult.ok) {
        this.handleParseError(parseResult, source);
        return;
      }

      await this.performValidation(parseResult, content, source);
    } catch (error) {
   ...
   */
  private handleParseError(parseResult: ParseResult, source: string): void {
    this.issues = [
      {
        severity: 'error',
        label: 'Invalid input',
    ...
    this.checkResults = [
      ...
      },
      {
        ...
        status: 'error',
        statusLabel: 'Error',
        detail: parseResult.error ?? 'Invalid JSON/NDJSON input.'
      }
    ];
  }

  /**
   * Performs validation on parsed resources
   */
  ...
  /**
   * Generates validation summary results
   */
  ...
  /**
   * Parses input content and extracts FHIR Observation resources
   */
  ...
  /**
   * Extracts Observation resources from parsed JSON
   */
  ...
  /**
   * Validates a single FHIR Observation resource
   */
